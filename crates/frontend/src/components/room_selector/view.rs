use super::view_model::RoomSelectorViewModel;
...
#[component]
pub fn RoomSelector(on_room_selected: Callback<String>, theme: Theme) -> impl IntoView {
    let i18n = use_i18n();
    let vm = RoomSelectorViewModel::new();

    let on_submit = move |ev: SubmitEvent| {
        ev.prevent_default();
        vm.submit(on_room_selected);
    };
            ...
            <div style=format!("background: {}; padding: 2.5rem; border-radius: 0.625rem; max-width: 25rem; width: 100%;", theme.ui_bg_primary)>
                ...
                <form on:submit=on_submit style="display: flex; flex-direction: column; gap: 1.25rem;">
                    <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                        ...
                        <input
                            type="text"
                            value=move || vm.room_id.get()
                            on:input=move |ev| vm.room_id.set(event_target_value(&ev))
